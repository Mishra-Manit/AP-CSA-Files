...
public class ElectionTesterV8 {
    ...
    public static void insertCandidateBefore(ArrayList <Candidate> candidateObjects, String beforeName, String addName, int addVotes){
        int location = 0;

        for (int i = 0; i < candidateObjects.size(); i++){
            if (candidateObjects.get(i).getName().equals(beforeName)){
                location = i;
            }
        }
    ...
    public static void deleteWithLoc(ArrayList <Candidate> candidateObjects, int position){
        candidateObjects.remove(position);
    }

    public static void deleteWithName(ArrayList <Candidate> candidateObjects, String name){
        int location = 0;
        int index;

        for (index = 0; index < candidateObjects.size(); index++){
            if (candidateObjects.get(index).getName().equals(name)){
                location = index;
                break;
            }
        }
        if (location != candidateObjects.size()){
            candidateObjects.remove(location);
        }
    }
}